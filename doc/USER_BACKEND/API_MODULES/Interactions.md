...
*   **Rate Limiting**:
    *   Для эндпоинтов лайков и комментариев применяются строгие лимиты через `slowapi` (Redis):
        *   Комментарии: 5 запросов/минуту
        *   Лайки: 10 запросов/минуту
        *   Просмотры: 20 запросов/минуту
    *   Это предотвращает автоматизированный спам и накрутку счетчиков.
*   **XSS Protection (Комментарии)**:
    *   Входящий HTML в комментариях очищается (Sanitization) с помощью библиотеки `bleach`.
    *   Разрешённые теги: `b`, `i`, `u`, `em`, `strong`, `a`, `br`, `p`.
    *   Разрешённые атрибуты: `href`, `title` для ссылок.
    *   Все остальные теги и скрипты удаляются для предотвращения Stored XSS атак.
*   **Приватность (GDPR)**:
